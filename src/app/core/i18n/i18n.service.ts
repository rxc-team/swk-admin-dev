 ...
 * @LastEditors: RXC 陈辉宇
 * @LastEditTime: 2020-06-18 11:22:28
 */
// 第三方库
...
import { CustomerService, LanguageService } from '@api';
...
export class I18NService {
  ...
  async updateDynamicLangData(lang?: string) {
    ...
    if (token) {
      // 获取语言
      await this.customer.getCustomers().then(async (customers: any[]) => {
        if (customers) {
          const cs = customers.map(async a => this.languageService.getLanguageData(lang || this.currentLang, a.domain));
          await forkJoin(cs)
            .toPromise()
            .then((data: any[]) => {
              if (data) {
  ...
  async switchLanguage(lang: string) {
    const lans = this._langs.map(item => item.code);
    lang = lans.find(n => n === lang) ? lang : lans[0];
    await this.use(lang);
    // 设置语言到store中
    await this.tokenService.updateUser({ language: lang });
